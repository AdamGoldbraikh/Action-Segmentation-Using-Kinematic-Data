...
class Trainer:
    def __init__(self, num_layers_PG, num_layers_R, num_R, num_f_maps, dim, num_classes_list, offline_mode=False, tau=16, lambd=0.15,hidden_dim_rnn=64,num_layers_rnn=3,secondary_sampling=2, dropout_TCN=0.5,dropout_RNN=0.5, task="gestures", device="cuda",
                 network='MS-TCN2',dataset=None,hyper_parameter_tuning=False,debagging=False,ToPrint=""):
        ...
        if network == 'MS-TCN2':
            ...
            self.model = MT_RNN("LSTM", input_dim=dim, hidden_dim=hidden_dim_rnn, num_classes_list=num_classes_list,
                                bidirectional=offline_mode, dropout=dropout_RNN,num_layers=num_layers_rnn)

        elif network == 'GRU':
            self.model = MT_RNN("GRU", input_dim=dim, hidden_dim=hidden_dim_rnn, num_classes_list=num_classes_list,
                                bidirectional=offline_mode, dropout=dropout_RNN,num_layers=num_layers_rnn)
        # elif network == 'MS-LSTM-TCN':
        #     self.model = RNN_MST_TCN2("LSTM",256, num_layers_R, num_R, num_f_maps,dropout, dim, num_classes_list, offline_mode,dropout_TCN=0.5,dropout_RNN=0.9)
        elif network == 'MS-TCN-LSTM':
            self.model = MST_TCN2_RNN(RNN_type='LSTM',num_layers_PG=num_layers_PG,hidden_dim=hidden_dim_rnn,num_R= num_R,num_f_maps= num_f_maps, dim=dim,dropout_TCN=dropout_TCN,dropout_LSTM=dropout_RNN, num_classes_list=num_classes_list,secondary_sampling=secondary_sampling, offline_mode=offline_mode)
        ...
        #     self.model = RNN_MST_TCN2("GRU",256, num_layers_R, num_R, num_f_maps, dim, num_classes_list, offline_mode,dropout_TCN=0.5,dropout_RNN=0.9)
            ...
            self.model = MST_TCN2_RNN(RNN_type='GRU',num_layers_PG=num_layers_PG,hidden_dim=hidden_dim_rnn,num_R= num_R,num_f_maps= num_f_maps, dim=dim,dropout_TCN=dropout_TCN,dropout_LSTM=dropout_RNN, num_classes_list=num_classes_list,secondary_sampling=secondary_sampling, offline_mode=offline_mode)
    ...
    def train(self, save_dir, batch_gen, num_epochs, batch_size, learning_rate, eval_dict, args):
        ...
        if self.dataset == "JIGSAWS":
            optimization_metric = 'Acc gesture'
        else:
            optimization_metric = 'F1-macro gesture'
        ...
        for epoch in range(num_epochs):
            ...
            if (epoch+1) % eval_rate == 0:
                ...
                if self.task == "gestures":
                   if results[optimization_metric] >= Max_metric:
                    Max_metric = results[optimization_metric]  # 'Acc gesture'  'F1-macro gesture'
                ...
                if self.task == "tools":
                   if (results['F1-macro left']  + results['F1-macro right'])/2 >= Max_metric:
                    Max_metric = (results['F1-macro left']  + results['F1-macro right'])/2
                    best_valid_results = results
                    if not self.debagging and not self.hyper_parameter_tuning:
                        torch.save(self.model.state_dict(), save_dir + "/"+self.network+"_"+self.task + ".model")
                        torch.save(optimizer.state_dict(), save_dir + "/"+self.network+"_"+self.task + ".opt")
                ...
                if self.task == "multi-taks":
                   if (results['F1-macro gesture'] + results['F1-macro left'] + results['F1-macro right'])/3 >= Max_metric:
                    Max_metric =(results['F1-macro gesture'] + results['F1-macro left'] + results['F1-macro right'])/3
                    best_valid_results = results
                    if not self.debagging and not self.hyper_parameter_tuning:
                        torch.save(self.model.state_dict(), save_dir + "/"+self.network+"_"+self.task + ".model")
                        torch.save(optimizer.state_dict(), save_dir + "/"+self.network+"_"+self.task + ".opt")
    ...
    def evaluate(self, eval_dict, batch_gen,is_test=False):
        print(colored(self.ToPrint, "blue"))
        results = {}
